...
from buddy.heuristics import RA
from buddy.utils import (
    ROOT_DIR,
    get_src_dst_degree,
    get_pos_neg_edges,
...
class HashDataset(Dataset):
    ...
    def _preprocess_node_features(self, data, edge_index, edge_weight, sign_k=0):
        ...
        if self.load_features and os.path.exists(feature_name):
            ...
            print(
                "Preprocessed features in: {:.2f} seconds".format(time() - start_time)
            )
        ...
        if not found_subgraph_features:
            ...
            if self.load_hashes and os.path.exists(hash_name):
                ...
                if os.path.exists(cards_name):
                    ...
                    print(
                        f"hashes found at {hash_name}, but cards not found. Delete hashes and run again"
                    )
            else:
                ...
                print(
                    "Preprocessed hashes in: {:.2f} seconds".format(time() - start_time)
                )
                if self.load_hashes:
            ...
            print(
                "Preprocessed subgraph features in: {:.2f} seconds".format(
                    time() - start_time
                )
            )
            assert self.subgraph_features.shape[0] == len(
...
def get_hashed_train_datasets(dataset, train_data, args, directed=False):
    root = f"{dataset.root}/elph_"
    use_coalesce = True if args.dataset_name == "ogbl-collab" else False
    pos_train_edge, neg_train_edge = get_pos_neg_edges(train_data)
    train_dataset = HashDataset(
...
def get_hashed_train_val_datasets(dataset, train_data, val_data, args, directed=False):
    root = f"{dataset.root}/elph_"
    use_coalesce = True if args.dataset_name == "ogbl-collab" else False
    pos_train_edge, neg_train_edge = get_pos_neg_edges(train_data)
    pos_val_edge, neg_val_edge = get_pos_neg_edges(val_data)
