...
from buddy.evaluation import evaluate_auc, evaluate_hits, evaluate_mrr
from buddy.utils import get_num_samples
...
def test(
    ...
    if eval_metric == "hits":
        ...
        results = evaluate_auc(val_pred, val_true)
...
def get_preds(model, loader, device, args, emb=None, split=None):
    ...
    print(
        f"{len(pos_pred)} positives and {len(neg_pred)} negatives for sample of {samples_used} edges"
    )
