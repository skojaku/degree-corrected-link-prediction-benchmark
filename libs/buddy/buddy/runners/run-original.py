            ...
            if (epoch + 1) % args.eval_steps == 0:
                ...
                for key, result in results.items():
                    ...
                    if args.wandb:
                        wandb.log(res_dic)
                    to_print = (
                        f"Epoch: {epoch:02d}, Best epoch: {best_epoch}, Loss: {loss:.4f}, Train: {100 * train_res:.2f}%, Valid: "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Efficient Link Prediction with Hashes (ELPH)"
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=inf,
        help="the number of training edges or % if < 1",
    )
    parser.add_argument(
        "--val_samples",
        type=float,
        default=inf,
        help="the number of val edges or % if < 1",
    )
    parser.add_argument(
        "--test_samples",
        type=float,
        default=inf,
        help="the number of test edges or % if < 1",
    )
    parser.add_argument("--preprocessing", type=str, default=None)
    parser.add_argument("--sign_k", type=int, default=0)
    parser.add_argument(
        "--load_features", action="store_true", help="load node features from disk"
    )
    parser.add_argument(
        "--load_hashes", action="store_true", help="load hashes from disk"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=inf,
        help="the number of training edges to cache",
    )
    parser.add_argument(
        "--year", type=int, default=0, help="filter training data from before this year"
    )
    # GNN settings
    ...
    parser.add_argument(
        "--save_model",
        action="store_true",
        help="save the model to use later for inference",
    )
    parser.add_argument(
        "--feature_prop",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="edge",
        help="how SEAL pools features in the subgraph",
    )
    # Subgraph settings
    ...
    parser.add_argument(
        "--weight_decay", type=float, default=0, help="Weight decay for optimization"
    )
    parser.add_argument("--epochs", type=int, default=100)
    parser.add_argument("--num_workers", type=int, default=4)
    parser.add_argument(
        "--num_negs", type=int, default=1, help="number of negatives for each positive"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="whether to add resource allocation features",
    )
    # SEAL specific args
    parser.add_argument(
        "--dynamic_train",
    ...
    parser.add_argument(
        "--eval_metric", type=str, default="hits", choices=("hits", "mrr", "auc")
    )
    ...
    parser.add_argument(
        "--hll_p", type=int, default=8, help="the hyperloglog p parameter"
    )
    parser.add_argument(
        "--minhash_num_perm", type=int, default=128, help="the number of minhash perms"
    )
    parser.add_argument(
        "--max_hash_hops",
        type=int,
        default=2,
        help="the maximum number of hops to hash",
    )
    parser.add_argument(
        "--subgraph_feature_batch_size",
        type=int,
        default=11000000,
    ...
    parser.add_argument(
        "--wandb_offline", dest="use_wandb_offline", action="store_true"
    )  # https://docs.wandb.ai/guides/technical-faq

    parser.add_argument(
        "--wandb_sweep", action="store_true", help="flag if sweeping"
    )  # if not it picks up params in greed_params
    parser.add_argument(
        "--wandb_watch_grad",
        action="store_true",
        help="allows gradient tracking in train function",
    )
    ...
    parser.add_argument(
        "--wandb_group", default="testing", type=str, help="testing,tuning,eval"
    )
    parser.add_argument("--wandb_run_name", default=None, type=str)
    parser.add_argument(
        "--wandb_output_dir",
        default="./wandb_output",
        help="folder to output results, images and model checkpoints",
    )
    parser.add_argument(
        "--wandb_log_freq", type=int, default=1, help="Frequency to log metrics."
    )
    parser.add_argument(
        "--wandb_epoch_list",
        nargs="+",
    ...
    parser.add_argument(
        "--log_features", action="store_true", help="log feature importance"
    )
